...
import { NotesClient } from '../../ui/src/notes-client.js';
import { NotesStore } from '../../ui/src/notes-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/notes_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new NotesStore(
  	new NotesClient(player.appWs as any, 'notes_test'),
  );
  return {
  	store,
  	player,
